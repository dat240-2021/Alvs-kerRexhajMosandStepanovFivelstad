...
BASEFOLDER = '.\\Infrastructure\\Data\\project_images\images\scattered images\\task 2 scattered images'
...
def insert_image_files(last_id, folder_name):
   ...
   for i in range(len(file_list)):
       if file_list[i].endswith('.png'):
           ...
           curr.execute("INSERT INTO IMAGEPIECE (ImageData, ImageId, SequenceNumber) VALUES (?, ?,?)", (binaryData, last_id, sequenceNumber))

def insert_categories(category_map):
    category_db_ids = {}
    for key in category_map:
        curr.execute("INSERT INTO IMAGECATEGORY (Category) VALUES (?)", (key,))
...
def execute_sql_statements(image_info,category_map):
    ...
    for i in range(len(image_info)):
        ...
        curr.execute("INSERT INTO IMAGE (ImageName, CategoryId) VALUES (?, ?)", (image_name,category_id))
...
if __name__ == '__main__':
    file1 = '.\\Infrastructure\\Data\\project_images\images\image_mapping.csv'
    image_list = read_csv_image_mapping(file1)
    file2 = '.\\Infrastructure\\Data\\project_images\images\label_mapping_with_categories.csv'
